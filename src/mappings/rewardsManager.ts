...
import { Indexer, Allocation, SubgraphDeployment, GraphNetwork, Provision } from '../types/schema'
import {
  RewardsAssigned,
...
function processRewardsAssigned(
  ...
  let indexerIndexingRewards =
    indexer.delegatedTokens == BigInt.fromI32(0)
      ? amount
      : amount.times(BigInt.fromI32(indexer.legacyIndexingRewardCut)).div(BigInt.fromI32(1000000))
  ...
  let epoch = createOrLoadEpoch(
    addresses.isL1 ? blockNumber : graphNetwork.currentL1BlockNumber!,
    graphNetwork,
  )
  ...
  getAndUpdateIndexerDailyData(indexer as Indexer, blockTimestamp)
  getAndUpdateSubgraphDeploymentDailyData(subgraphDeployment as SubgraphDeployment, blockTimestamp)
  getAndUpdateGraphNetworkDailyData(graphNetwork as GraphNetwork, blockTimestamp)
}
